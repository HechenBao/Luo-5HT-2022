function [] = indv_FLX(path_data)  % function for loading raw data and pre-processing
...
FLX = dir(fullfile(path_data,['*_FLX_*.mat']));            % find .mat file contains "FLX"
load([path_data,'/',FLX.name]);         % load to workspace
coG_FLX_raw = coef(1,:);                % get stress coef for G signal
coTd_FLX_raw = coef(2,:);               % get stress coef for Td signal
...
save(fullfile(path_data,[name '_raw' '.mat']),'coG_base_raw','coTd_base_raw',...
    'coG_FLX_raw','coTd_FLX_raw')       % save useable variables
...
coG_FLX = getround(coG_FLX_raw,10);               % get the round data length for downsampling    
coG_FLX = sepblockfun(coG_FLX,[1,10],'mean');     % downsampled to 1 Hz

coTd_FLX = getround(coTd_FLX_raw,10);               
coTd_FLX = sepblockfun(coTd_FLX,[1,10],'mean');
...
stitch_coG = [coG_base,coG_FLX];        % stitch baseline and FLX data
stitch_coTd = [coTd_base,coTd_FLX];     % stitch baseline and FLX data
...
figure('Position', [100 100 900 600])    
    ...
    phase2 = phase1+length(coG_FLX);       
    plot(time(1:phase1),diff(1:phase1),'k',...
    time(phase1:phase2),diff(phase1:phase2), 'b')
...
hold on
H_FLX = histogram(diff_FLX,'BinEdges',bins,'Normalization','probability',...
...
% [output_binFLX,output_binFLX_mean] = bindata(diff_FLX,binsize); 
...
% output_dir = ('D:\5HT sensor\GRAB5HT photometry\Output\FLX');
% save([output_dir,'FLX_male.mat'],'-append')
